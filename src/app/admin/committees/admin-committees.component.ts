...
const DESIGNATIONS = [
  ...
  'Treasurer', 'Assistant Treasurer', 'Executive Member', 'Adviser',
];
...
export class AdminCommitteesComponent implements OnInit {
  ...
  load(): void {
    ...
    this.adminService.adminListCommittees().subscribe({
      next: (data) => { this.committees.set(data); this.loading.set(false); },
      error: (err) => {
        this.error.set(err?.status === 403 ? "You don't have permission to view this." : 'Failed to load committees.');
  ...
  saveCommittee(): void {
    ...
    if (!term || !sessionLabel) { this.error.set('Term and session label are required.'); return; }
    ...
    req.subscribe({
      ...
      error: (err) => {
        this.error.set(err?.status === 403 ? "You don't have permission." : (err.error?.message ?? 'Failed to save.'));
        this.saving.set(false);
      },
    });
  }
  ...
  deleteCommittee(c: Committee): void {
    if (!confirm(`Delete committee "${c.sessionLabel}"? This will also remove all members.`)) return;
    ...
    this.adminService.adminDeleteCommittee(c.id).subscribe({
      next: () => {
        ...
        this.deleting.update((s) => { const n = new Set(s); n.delete(c.id); return n; });
  ...
  addMember(): void {
    ...
    this.adminService.adminAddCommitteeMember(committeeId, {
      userId,
      designation,
      ...
      next: (newEntry) => {
        this.committees.update((list) =>
          list.map((c) => c.id === committeeId ? { ...c, members: [...c.members, newEntry] } : c),
        );
      ...
      error: (err) => {
        this.error.set(err?.status === 403 ? "You don't have permission." : (err.error?.message ?? 'Failed to add member.'));
        this.addingMember.set(false);
      },
    });
  }
  ...
  removeMember(entry: CommitteeEntry): void {
    if (!confirm(`Remove ${entry.user.displayName || entry.user.email} from this committee?`)) return;
    const committeeId = this.selected()!.id;
    ...
    this.adminService.adminRemoveCommitteeMember(entry.id).subscribe({
      next: () => {
        ...
        this.removingMemberIds.update((s) => { const n = new Set(s); n.delete(entry.id); return n; });
        this.success.set('Member removed.');
      },
      error: (err) => {
        this.error.set(err?.status === 403 ? "You don't have permission." : 'Failed to remove member.');
        this.removingMemberIds.update((s) => { const n = new Set(s); n.delete(entry.id); return n; });
      },
    });
  }

  isRemovingMember(id: string): boolean { return this.removingMemberIds().has(id); }

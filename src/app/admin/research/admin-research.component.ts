...
export class AdminResearchComponent implements OnInit {
  ...
  load(): void {
    ...
    this.error.set('');
    this.adminSvc.listResearchPapers().subscribe({
      next: (data) => {
  ...
  openEdit(p: AdminResearchPaper): void {
    ...
    this.formFeatured.set(p.featured);
    this.error.set('');
    this.success.set('');
    this.mode.set('edit');
  }
  ...
  save(): void {
    ...
    const req =
      id === null
        ? this.adminSvc.adminCreateResearchPaper(dto)
        : this.adminSvc.adminUpdateResearchPaper(id, dto);

    req.subscribe({
      next: (saved) => {
        if (id === null) {
          this.papers.update((list) => [saved, ...list]);
          this.success.set('Research paper created.');
        } else {
      ...
      error: (err) => {
        this.error.set(
          err?.status === 403
            ...
            : (err?.error?.message ?? 'Failed to save research paper.'),
        );
        this.saving.set(false);
      },
    });
